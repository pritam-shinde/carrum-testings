            ...
            <ul onClick={handleOnClick} className={`${click ? 'dropdownMenu clicked' : 'dropdownMenu'}   shadow p-3 bg-white`}>
                {
                    ...
                    {
                        id: "service-menu-9", title: "INVISALIGN", path: "/service/invisalign/", submenu: [
                            { id: "services-menu-9.1", link: "/service/invisalign-open-day/", text: "INVISALIGN OPEN DAY" },
                        ]
                    },
                    ...
                    { id: "service-menu-17", title: "SLEEP DENTISTRY", path: "/service/sleep-dentistry/", submenu: null }].map(item => <li className="nav-item mb-2" key={item.id} onClick={() => { setClick(false); closeMobileMenu() }}>
                            ...
                            item.submenu !== null ? <ul className='ps-3 ms-3' style={{ borderLeft: "1px solid #555" }}>
                                {
                                    item.submenu.map(menu => <li className="nav-item mb-2" key={menu.id} onClick={() => { setClick(false); closeMobileMenu() }}>
                                        <a href={menu.link} className='nav-link text-dark'>{menu.text}</a>
                                    </li>)
                                }
                            </ul> : null
                        }
                        ...
                        <a href={item.path} className='nav-link text-dark p-0'>{item.title}</a>
                    </li>) : null
                }
            </ul >
        </>
